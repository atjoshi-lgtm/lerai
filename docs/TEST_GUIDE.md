...
```bash
python3 -m unittest tests.test_toml_generator_comment_preservation tests.test_toml_generator tests.test_entity_extractor_normalization tests.test_conflict_detector_object_count
```
...
| `tests/test_toml_generator.py` | Verifies `execute_ast_update` deletion matching normalizes scalar types and formatting (for example integer `Region-number` values and padded/cased `Mapnames`) so exact-match deletion works across JSON-vs-TOML type differences. |
